...
sdlmsg_replay_init(void *arg, void(*pCallback)(struct timeval)) {
...
#elif defined __APPLE__
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
    ga_logger(Severity::INFO, "sdl replayer: iOS is not supported now\n");
#else
    do {
    ...
    ctrl_server_set_resolution(cxsize, cysize);
    ctrl_server_set_output_resolution(cxsize, cysize);
    ga_logger(Severity::INFO, ("sdl replayer: Replay using CGPostEvent(), screen-size=%dx%d\n",
        cxsize, cysize);
#endif
#elif defined ANDROID
    ga_logger(Severity::INFO, "sdl replayer: Android is not supported now\n");
#else    // X11
    int evtbase, errbase, major, minor;
    if((display = XOpenDisplay(conf->display)) == NULL) {
        ga_logger(Severity::ERR, "sdl replayer: cannot open display '%s'\n",
            conf->display);
        return -1;
    }
    //
    ...
    if(XTestQueryExtension(display, &evtbase, &errbase, &major, &minor) == False) {
        ga_logger(Severity::ERR, "sdl replayer: XTest not supported.\n");
        return -1;
    }
    ga_logger(Severity::INFO, "sdl replayer: Replay using XTest (version %d.%d) for display %s screen %d, size=%dx%d.\n",
