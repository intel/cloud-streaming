...
encoder_ptv_get(unsigned queueid, long long pts, struct timeval *ptv, int interpolation) {
    ...
    while(pts_queue[queueid].size() > 0) {
        ...
        if(interpolation > 0) {
            ...
            *ptv = pts_queue[queueid].front().ptv;
            ptv->tv_sec -= static_cast<long>(delta_tv / 1000000LL);
            delta_tv %= 1000000LL;
            if(ptv->tv_usec < delta_tv) {
                ptv->tv_sec--;
                ptv->tv_usec += 1000000LL;
            }
            ptv->tv_usec -= static_cast<long>(delta_tv);
            return ptv;
        }
        break;
    }
#if 1
    ga_logger(Severity::ERR, "FIXME: encoder_ptv_get failed: id=%d, pts=%lld\n", queueid, pts);
#endif
    return NULL;
}
...
encoder_pktqueue_init(int channels, int qsize) {
    ...
    for(i = 0; i < channels; i++) {
        ...
        if((pktqueue[i].buf = (char *) malloc(qsize)) == NULL) {
            ga_logger(Severity::ERR, "encoder: initialized packet queue#%d failed (%d bytes)\n",
                i, qsize);
            exit(-1);
        }
    ...
    ga_logger(Severity::INFO, "encoder: packet queue initialized (%dx%d bytes)\n", channels, qsize);
    return 0;
}
...
encoder_pktqueue_reset_channel(int channelId) {
    std::lock_guard<std::mutex> lock(pktqueue[channelId].mutex);
 ...
 */
int
encoder_pktqueue_append(int channelId, ga_packet_t *pkt, int64_t encoderPts, struct timeval *ptv) {
...
size_check:
    ...
    if(q->datasize + pkt->size > q->bufsize) {
        ga_logger(Severity::ERR, "encoder: packet queue #%d full, packet dropped (%d+%d)\n",
 ...
 *
 * This function ONLY reads the first packet.
 ...
 * the value as the \a offset parameter to this function.
 * When this function returns, the first packet in the queue would hold
 * exact \a N bytes and the rest \a (M-N) bytes would be included
...
encoder_pktqueue_split_packet(int channelId, char *offset) {
    ...
    newpkt = *pkt;
    newpkt.size = static_cast<unsigned int>(offset - pkt->data);
    newpkt.padding = 0;
    //
    pkt->data = offset;
...
encoder_pktqueue_register_callback(int channelId, qcallback_t cb) {
    ...
    ga_logger(Severity::INFO, "encoder: pktqueue #%d callback registered (%p)\n", channelId, cb);
    return 0;
}
