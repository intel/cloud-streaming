    ...
    *token++ = '\0';
    ...
    if((leftbracket = strchr(option, '[')) != NULL) {
        ...
        if(rightbracket == NULL) {
            ga_logger(Severity::ERR, "# %s:%d: malformed option (%s without right bracket).\n",
                filename, lineno, option);
            return -1;
        }
        // no key specified
        if(leftbracket + 1 == rightbracket) {
            ga_logger(Severity::ERR, "# %s:%d: malformed option (%s without a key).\n",
                filename, lineno, option);
            return -1;
        }
        // garbage after rightbracket?
        if(*(rightbracket+1) != '\0') {
            ga_logger(Severity::ERR, "# %s:%d: malformed option (%s?).\n",
                filename, lineno, option);
            return -1;
        }
        *leftbracket = '\0';
 ...
 */
char *
ga_conf_readv(const char *key, char *store, size_t slen) {
    ...
    if (mi->second.value().length() >= slen) {
        return NULL;
    }
...
/**
 * Load the value of a parameter in a std::string format.
 *
 * @param key [in] The parameter to be loaded.
 * @return An std::string containing loaded value or empty string on the failure.
 */
...
ga_conf_readint(const char *key) {
    std::string str = ga_conf_readstr(key);
    ...
    char *endptr = nullptr;
    errno = 0;
    const long sl = strtol(str.c_str(), &endptr, 0);
    if (errno == ERANGE || sl > INT_MAX || sl < INT_MIN) {
        ga_logger(Severity::ERR, "%ld is out of integer range\n", sl);
        return 0;
    }
    else if (endptr == str.c_str()) {
        ga_logger(Severity::ERR, "%s: not a decimal number\n", str.c_str());
        return 0;
    }
    else if (nullptr == endptr) {
        ga_logger(Severity::ERR, "endptr is nullptr\n");
        return 0;
    }

    if ('\0' != *endptr) {
        ga_logger(Severity::WARNING, "%s : Further characters after number %s\n", str.c_str(), endptr);
    }
...
ga_conf_readdouble(const char *key) {
    ...
    char *endptr = nullptr;
    errno = 0;
    const long double sd = strtod(str.c_str(), &endptr);
    if (errno == ERANGE) {
        ga_logger(Severity::ERR, "%s : double range overflow or underflow\n", str.c_str());
        return 0.0;
    }
    else if (nullptr == endptr) {
        ga_logger(Severity::ERR, "endptr is nullptr\n");
...
ga_conf_readints(const char *key, int *val, int n) {
    ...
    if(ptr == NULL)
        return 0;
    return ga_conf_multiple_int(ptr, val, n);
}
...
ga_conf_readbool(const char *key, int defval) {
    ...
    if(str.empty())
        return defval;
    return ga_conf_boolval(str.c_str(), defval);
}
 ...
 *
 * @param mapname [in] The name of the parameter map.
 * @param keystore [out] The buffer used to store the current key.
 * @param klen [in] The buffer length of \a keystore.
 ...
 *
 * @param mapname [in] The name of the parameter map.
 * @param valstore [out] The buffer used to store the current key.
 * @param vlen [in] The buffer length of \a valstore.
 ...
 *
 * @param mapname [in] The name of the parameter map.
 * @param keystore [out] The buffer used to store the next key.
 * @param klen [in] The buffer length of \a keystore.
 *
 * This function is usually called during an enumeration process.
 * Note that if \a keystore is NULL, this function automatically allocates
 * spaces to store the key. You may need to release the memory by
 * calling \em free().
 */
...
/**
 * Get the current key of the global runtime configuration.
 *
...
/**
 * Advance and get the key of the global runtime configuration.
 *
