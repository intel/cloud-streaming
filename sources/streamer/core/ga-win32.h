...
EXPORT std::string HRESULT_to_string(HRESULT hr);
EXPORT std::string LastError_to_string(DWORD dwErr);
EXPORT std::wstring IID_to_wstring(const IID& riid);

EXPORT HWND GetWindowHandleOfCurrentProcess();

#endif    /* USE_GA_WIN32_MACRO */

EXPORT bool RunSystemCommand(const std::string& cmdline, std::string& result);

EXPORT unsigned int get_adapter_index();
EXPORT bool init_adapters_luid_map();
